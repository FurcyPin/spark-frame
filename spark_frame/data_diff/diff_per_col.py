...
def _get_pivoted_df(
    top_per_col_state_df: DataFrame, max_nb_rows_per_col_state: int,
) -> DataFrame:
    ...
    pivoted_df = (
        ...
        .pivot(
            "state", values=["changed", "no_change", "only_in_left", "only_in_right"],
        )
        .agg(
            f.sum("nb").alias("nb"),
            f.expr(
...
def _format_diff_per_col_df(pivoted_df: DataFrame, col_df: DataFrame) -> DataFrame:
    ...
    total_col = (
        f.col("changed_nb")
        + f.col("no_change_nb")
        + f.col("only_in_left_nb")
        + f.col("only_in_right_nb")
    )
