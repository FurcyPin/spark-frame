...
class DiffStats:
    ...
    def in_both(self) -> int:
        return self.total - self.only_in_left - self.only_in_right
...
def print_diff_stats_shard(diff_stats: DiffStats, left_df_alias: str, right_df_alias: str) -> None:
    ...
    if nb_row_diff != 0:
        ...
        print(f"Row count ok: {diff_stats.in_right} rows")
        print("")
    ...
    if diff_stats.only_in_right > 0:
        print(f"{diff_stats.only_in_right} ({diff_stats.percent_only_in_right}%) rows are only in '{right_df_alias}")
    print("")
