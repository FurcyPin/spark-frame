...
DEFAULT_GROUP_ALIAS = "group"


def _remove_first_dot(s: str):
    if s.startswith("."):
        return s[1:]
    else:
        return s
...
def analyze(
    ...
    group_alias: str = DEFAULT_GROUP_ALIAS,
    ...
    (col: str, schema_field: StructField, col_num: int) -> Column

    Examples of aggregation methods can be found in the module :module:`spark_frame.transformation_impl.analyze_aggs`
    ...
    Examples:
        ...
        <BLANKLINE>
    """
    # TODO: add checks when column names do not exist in DataFrame
    if _aggs is None:
        _aggs = default_aggs
    if group_by is None:
        group_by = []
