...
def unnest_fields(
    ...
    grouped_dataframes_with_columns: Dict[DataFrame, List[Column]] = group_by_key(dataframe_and_columns)

    dataframes_with_selected_columns: List[DataFrame] = [
        df.select(
            *_get_keep_columns_for_final_select(df, cols, keep_columns_list),
            *cols,
        )
        for df, cols in grouped_dataframes_with_columns.items()
    ]
    return {_get_deepest_unnested_field(df.columns): df for df in dataframes_with_selected_columns}
